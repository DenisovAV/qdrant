...
void main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    if (targetOS == OS.current && inMonorepo) {
      ...
      // edge/segment/shard/sparse crates, so editing those (or the lockfile)
      // must invalidate the cached dylib or `dart test` silently passes stale.
...
Future<Uri> _download(String dirName, String libFileName) async {
  ...
  try {
    ...
    await resp.pipe(tmpArchive.openWrite());
  } finally {
    client.close();
  }
